		...
		public override void UseGate( Mobile m )
		{
			...
			else
			{
				...
				if (_ChangesSkills)
				{
					...
					for ( int i = 0; i < skills.Length; ++i ) try { skills[i].Base = gAllSkills[i]; } catch {}
				}
				
				if (_ChangesCaps)
				{
					Server.Skills skills = m.Skills;
					for ( int i = 0; i < skills.Length; ++i ) try { skills[i].Cap = gAllSkillCaps[i]; } catch {}
				}
				...
				if (_SkillsGiveItems)
				{
					...
					for ( int i = 0; i < s.Length; ++i )
						try { if (gAllSkills[i] >= _SkillsItemsMin)
