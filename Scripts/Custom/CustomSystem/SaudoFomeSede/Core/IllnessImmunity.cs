	...
	public static class IllnessImmunity
	{
		...
		public static void TryAttackImmune(ref PlayerMobile player)
		{
			...
			if (Utility.Random(0, 10) > immunity)
			{
				...
				if (infectionChance >= Utility.Random(immunity, immunity * 10))
				{
					...
					if (IllnessHandler.InDebug)
					{
						var msg = $"[ Atacado pelo Sistema Imunológico: {player.Name} pegou alguma coisa! ]";
		...
		public static int CheckHungerThirst(PlayerMobile player)
		{
			if (player.Thirst < 10)
			{
				if (player.Thirst < 5)
				{
					player.Say("*Labios Racham*");

					player.SendMessage("Você está precisando urgentemente de um pouco de água!");
				}
				else
				{
					player.Say("*Boca Seca*");

					player.SendMessage("Você está precisando de um pouco de água!");
				}
			}
			...
			if (player.Hunger < 10)
			{
				if (player.Hunger < 5)
				{
					player.Say("*Estomago Revira*");

					player.SendMessage("Você está precisando urgentemente de um pouco de comida!");
				}
				else
				{
					player.Say("*dor de estômago*");

					player.SendMessage("Você está precisando de um pouco de comida!");
				}
			}
